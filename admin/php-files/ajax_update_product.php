...
if (isset($_POST['product_id'])) {
    ...
    if ($product_image_name == "") {
        ...
        $insertQuery = "UPDATE products SET product_name = ?, product_desc = ?, product_price = ? WHERE product_id = ?";
        $insertStmt = $conn->prepare($insertQuery);
        $result = $insertStmt->execute([$product_name, $product_desc, $product_price, $product_id]);
        ...
        if ($isValidType && $img_size < 5242880) {
            ...
            $insertQuery = "UPDATE products SET product_name = ?, product_desc = ?, product_price = ?, product_image = ? WHERE product_id = ?";
            $insertStmt = $conn->prepare($insertQuery);
            $result = $insertStmt->execute([$product_name, $product_desc, $product_price, $product_image_name, $product_id]);
