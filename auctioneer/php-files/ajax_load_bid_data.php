...
if (isset($_SESSION['user_id'])) {
    ...
    while ($bid_data = $select_bid_stmt->fetch(PDO::FETCH_OBJ)) {

        $bid_user_id = $bid_data->bid_user_id;
        $bid_amount = $bid_data->bid_amount;
        $bid_product_id =  $bid_data->bid_product_id;
        $bid_date =  date("d-M-Y", strtotime($bid_data->bid_date));;
        ...
        $select_product_query = "SELECT * FROM products WHERE product_id = ? AND product_user_id = ? AND !winner > 0";
        $select_product_stmt = $conn->prepare($select_product_query);
        $select_product_result = $select_product_stmt->execute([$bid_product_id, $user_id]);
        ...
        if ($select_product_stmt->rowCount() > 0) {
            ...
            $output .= "
                <tr>    
                    ...
                    <td>$bid_amount <i class='fas fa-rupee-sign'></i></td>
                    <td>$bid_date</td>
                    <td class='pr-1'>
...
//     $select_bid_query = "SELECT * FROM bids WHERE  = ?";
//     $select_bid_stmt = $conn->prepare($select_bid_query);
